...
resource "aws_s3_bucket_notification" "etl_trigger" {
  ...
  lambda_function {
    lambda_function_arn = aws_lambda_function.etl_customer_ledger.arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "raw/Ledger"
    filter_suffix       = ".xlsx"
  }
  ...
  lambda_function {
    lambda_function_arn = aws_lambda_function.etl_appendix_b_x11.arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "raw/Barcodes"
    filter_suffix       = ".xlsx"
  }

  lambda_function {
    lambda_function_arn = aws_lambda_function.etl_appendix_b_x11_purchase.arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "raw/AppendixPurchase"
    filter_suffix       = ".xlsx"
  }

  lambda_function {
    lambda_function_arn = aws_lambda_function.etl_appendix_b_x11_purchase_return.arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "raw/AppendixPurReturn"
    filter_suffix       = ".xlsx"
  }
  ...
  lambda_function {
    lambda_function_arn = aws_lambda_function.etl_supplier_accounts.arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "raw/Supplier"
    filter_suffix       = ".xlsx"
  }
  ...
  lambda_function {
    lambda_function_arn = aws_lambda_function.etl_borrowings.arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "raw/Borrowings"
    filter_suffix       = ".xlsx"
  }
  ...
  depends_on = [
    ...
    aws_lambda_permission.s3_invoke_etl_supplier_accounts,
    aws_lambda_permission.s3_invoke_whatsapp_notifier,
    aws_lambda_permission.s3_invoke_etl_borrowings,
  ]
}
