...
-- Admin-configured per-account interest rate on borrowings. Natural key: (account).
-- Uni-temporal milestoning: in_z/out_z track versions; out_z IS NULL = current record.
-- Modelled on monthly_sale_targets (migration 023). Table name is deliberately
-- singular (borrowing_rate), unlike most other tables in this repo, per explicit
-- request. (migration 054)
--
...
-- ============================================================
-- RBAC — dashboard access control (migration 009)
-- Roles, mappable screens, role→screen mappings, and login users.
-- password_hash is a PBKDF2 composite produced by the API Lambda
-- (business-core/lambda/api/auth.py). The bootstrap admin user is
-- created on first login, not seeded here.
-- ============================================================
...
-- Canonical mappable screens. Keys MUST match ui/src/screens.ts.
...
INSERT INTO app_screens (screen_key, label, sort_order) VALUES
    ...
    ('reports.monthly_sales',         'Monthly Sales',            92);
